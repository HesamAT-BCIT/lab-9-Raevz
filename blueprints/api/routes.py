...
MOCK_SENSOR_DATA_PATH = PROJECT_ROOT / "mock_sensor_data.json"
...
def api_create_profile(uid: str):
    """Create or replace the current user's profile from a JSON body."""
...
def api_get_sensor_data(uid: str):
    """Return local mock sensor data for the dashboard chart."""
    try:
        with MOCK_SENSOR_DATA_PATH.open("r", encoding="utf-8") as f:
            data = json.load(f)
